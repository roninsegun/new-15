/* [data-split] → SplitText, after the font is in (a split measured against
   ...
   `mask` wraps each unit in `.<unit>-mask`; style.css overrides its inline
   overflow:clip (`overflow: visible !important`) and clips with a clip-path
   that bleeds vertically instead, so poster-size descenders survive.
